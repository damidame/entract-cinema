...
class TagUseCase(
    ...
    suspend fun tagSchedule() {
        performTag { dataStore.tagSchedule() }
    }

    suspend fun tagEvent() {
        performTag { dataStore.tagEvent() }
    }

    suspend fun tagDetails(sessionId: String) {
        performTag { dataStore.tagDetails(sessionId) }
    }

    suspend fun tagCalendar(sessionId: String) {
        performTag { dataStore.tagCalendar(sessionId) }
    }

    private inline fun performTag(tag: () -> Unit) {
        if (!platformUtils.isDebug()) tag()
    }
}
